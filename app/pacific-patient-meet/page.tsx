...
interface Doctor {
  ...
  departments: Department[]
  popular?: boolean // Optional: Added for popular badge if needed
}
interface Branch {
  _id: string
  branchName: string
...
const DoctorCard = ({ doctor }: DoctorCardProps) => {
  ...
  const branchName = primaryLocation?.branchName || null
  const locationDisplay = primaryLocation 
    ? (branchName ? `${branchName}, ${primaryLocation.hospitalName}` : primaryLocation.hospitalName)
    : 'N/A'
      ...
      <article className="group bg-white rounded-xs shadow-xs transition-all duration-300 overflow-hidden cursor-pointer h-full flex flex-col hover:shadow-sm border border-gray-100">
        ...
        <div className="p-3 flex-1 flex flex-col space-y-2">
          ...
          <div className="space-y-2">
            <p className="text-sm text-gray-900 font-normal flex items-center gap-1.5">
              <div className="w-1.5 h-1.5 bg-[#74BF44] rounded-full flex-shrink-0"></div>
