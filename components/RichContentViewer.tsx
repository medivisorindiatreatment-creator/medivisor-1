...
export function RicosRenderer({ content }: RicosRendererProps) {
  console.log('[v0] RicosRenderer received content:', content)
  console.log('[v0] Content type:', typeof content)
  console.log('[v0] Is array:', Array.isArray(content))
  console.log('[v0] Has nodes:', content && 'nodes' in content)
  
  if (!content || (!content.nodes && !Array.isArray(content))) {
    console.warn('[v0] RicosRenderer received invalid content:', content)
    return null
  }

  const nodes = content.nodes || (Array.isArray(content) ? content : [])

  console.log('[v0] Processing nodes:', nodes)
  console.log('[v0] Nodes length:', nodes.length)

  if (nodes.length === 0) {
    console.warn('[v0] No nodes to render')
    return null
  }
  ...
  const renderNode = (node: any, index: number): React.ReactNode => {
    ...
    console.log('[v0] Rendering node:', node.type, node)
    ...
    if (node.type === 'IMAGE' || node.imageData) {
      ...
      // If imageSrc is an object with an 'id' property, extract the id
      if (imageSourceData && typeof imageSourceData === 'object' && imageSourceData.id) {
        imageSrc = imageSourceData.id
        console.log('[v0] Extracted image ID from object:', imageSrc)
      ...
      console.log('[v0] Processing image node:', { imageSrc, alt, width, height })
      ...
      if (imageSrc) {
        ...
        if (imageSrc.startsWith('http://') || imageSrc.startsWith('https://')) {
          ...
          try {
            ...
            finalImageUrl = url
            console.log('[v0] Converted Wix image URL:', finalImageUrl)
          } catch (error) {
            console.error('[v0] Error converting Wix image URL:', error)
            // Fallback parsing method
            const match = imageSrc.match(/wix:image:\/\/v1\/([^/]+)\//)
            if (match && match[1]) {
              finalImageUrl = `https://static.wixstatic.com/media/${match[1]}`
              console.log('[v0] Using fallback Wix image URL:', finalImageUrl)
            }
          }
        } else {
          // Regular Wix media ID, construct URL
          finalImageUrl = `https://static.wixstatic.com/media/${imageSrc}`
          console.log('[v0] Constructed Wix static URL:', finalImageUrl)
        }
      }
          ...
          <div key={index} className="my-8">
            <img 
              ...
              onError={(e) => {
                ...
                console.error('[v0] Failed to load image:', finalImageUrl)
    ...
    if (node.type === 'VIDEO' || node.videoData) {
      ...
      console.log('[v0] Processing video node:', { src, poster })
    ...
    if (node.type === 'BLOCKQUOTE' || node.blockquoteData) {
      return (
        <blockquote key={index} className="border-l-4 border-blue-500 pl-6 my-6 italic text-[#241d1f] bg-gray-50 py-4 rounded-r-lg">
          {node.nodes && renderNodes(node.nodes)}
        </blockquote>
      )
    }
      ...
      return (
        <pre key={index} className="bg-gray-900 text-gray-100 p-6 rounded-lg overflow-x-auto my-6">
          <code className="text-sm font-mono">
            {node.textData?.text || (node.nodes && renderNodes(node.nodes))}
          </code>
        </pre>
      )
    }
    ...
    console.warn('[v0] Unknown node type:', node.type, node)
    return null
  }
